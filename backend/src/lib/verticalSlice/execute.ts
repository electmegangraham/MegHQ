...
function buildSourceTrace(request: VerticalSliceRequest): Record<string, unknown> {
  return {
...
export async function executeVerticalSlice(request: VerticalSliceRequest): Promise<VerticalSliceResult> {
  ...
  const { data: task, error: taskError } = await db
    ...
    .insert({
      ...
      initiative_id: (initiative as InsertedInitiative).id,
      owner_id: request.ownerId,
      lead_department_id: request.leadDepartmentId,
      status: "accepted",
  ...
  const linkedArtifactIds =
    request.artifactId && isUuid(request.artifactId) ? [request.artifactId] : [];
  const linkedApprovalIds =
    request.approvalId && isUuid(request.approvalId) ? [request.approvalId] : [];
  ...
  const { data: deskItem, error: deskItemError } = await db
    ...
    .insert({
      ...
      status: "active",
      linked_object_ids: [(initiative as InsertedInitiative).id, (task as InsertedTask).id],
  ...
  const created: VerticalSliceResult["created"] = {
    signalId: request.signalId,
    initiativeId: (initiative as InsertedInitiative).id,
    taskId: (task as InsertedTask).id,
    deskItemId: (deskItem as InsertedDeskItem).id
  };
  ...
  return {
    ...
    created,
    states: {
      initiativeStatus: (initiative as InsertedInitiative).status,
      taskStatus: (task as InsertedTask).status,
      deskItemStatus: (deskItem as InsertedDeskItem).status
    },
