import type { AlertResult, DecisionInput, DecisionResult } from "./types.js";

export function scoreDecision(input: DecisionInput): DecisionResult {
  let score = 0;
  if (input.blocked) score += 50;
  if (input.requiresApproval) score += 25;
  if (input.priority === "P0") score += 20;
  if (input.priority === "P1") score += 10;
  if (input.urgency === "immediate") score += 15;
  if (input.urgency === "today") score += 8;
...
export function computeAlert(input: DecisionInput): AlertResult {
