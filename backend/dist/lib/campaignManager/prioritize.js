function rank(signal) {
    ...
    if (signal.urgency === "immediate")
        score += 20;
    if (signal.urgency === "today")
        score += 10;
    return score;
}
export function runCampaignManager(signals) {
    ...
    return {
        ...
        explainability: surfaced.map((s) => `${s.id}: blocked=${Boolean(s.isBlocked)}, approval=${Boolean(s.requiresApproval)}, priority=${s.priority ?? "none"}, urgency=${s.urgency ?? "none"}`),
